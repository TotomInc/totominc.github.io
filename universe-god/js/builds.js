...
game.builds.update = function() {
	for (var i = 0; i < g.b.list.length; i++) {
		$("#builds-infos-" + i).html(g.b.list[i].name + " : " + fix(g.b.list[i].reward, 0) + " " + g.b.list[i].rewardName.toLowerCase() + "/sec<br>" + fix(g.b.owned[i], 0) + " owned : " + fix(h.buildReward(i), 0) + " " + g.b.list[i].rewardName.toLowerCase() + "/sec" + "<br>Cost " + fix(h.buildPrice(i), 0) + " " + g.b.list[i].priceName.toLowerCase())
	};
};
